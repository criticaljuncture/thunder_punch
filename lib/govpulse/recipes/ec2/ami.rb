Capistrano::Configuration.instance(:must_exist).load do
  ...
  namespace :ec2 do
    ...
    task :load_ec2_config do
      alert_user('You need to set :ec2_config_location in your deploy file', :abort => true) unless :ec2_config_location
      #set :ec2_config_location, "#{RAILS_ROOT}/config/ec2_config.yml"
      if !File.exist?(ec2_config_location)
        raise "You must configure your ec2 settings in config/ec2_config.yml!"
      else
    ...
    task :copy_keys do
      load_ec2_config unless :ec2_config_loaded
      upload "#{aws_key_location}/#{ec2_private_key} #{ec2_x509_cert} #{user}@#{domain}:/mnt", :via => :scp
    end
  
    desc 'Bundle your AMI'
    task :bundle_ami do
      load_ec2_config unless :ec2_config_loaded
      alert_user("You need to set :bundle_name via the command line\n `cap ec2:bundle_ami -s bundle_name=sample`", :abort => true) unless configuration[:bundle_name]
      sudo "ec2-bundle-vol -d /mnt -k /mnt/#{ec2_private_key} -c /mnt/#{ec2_x509_cert} -u #{aws_account_id} -r #{ec2_architecture} -p #{bundle_name}"
    end
